    ...
    <form method="post" action="<?= "./updateArticle.php?id=" . $numArt; ?>" enctype="multipart/form-data" class="ui form">
        ...
        <div class="control-group">
            ...
            <?php 
                ...
                $group = $maThem->get_AllThematiquesByLang($row['numLang']);
            ?>
                <div class="field langSelect" id="them<?= $row['numLang'] ?>">
                    <label class="control-label" for="<?= $row['numLang'] ?>"><b>Thématique :</b></label>
                    <select class="themSelect" name="<?= $row['numLang'] ?>" id="<?= $row['numLang'] ?>">
                        <?php foreach ($allLangs as $raw) : ?>
                            <?php if ($raw["numThem"] === $numThem) : ?>
                                <option value="<?= $raw['numThem'] ?>" <?= ($raw["numThem"] === $numThem) ? 'selected' : '' ?>><?= $raw['libThem'] ?></option>
                            <?php endif ?>
                        <?php endforeach ?>
                    </select>
                </div>
